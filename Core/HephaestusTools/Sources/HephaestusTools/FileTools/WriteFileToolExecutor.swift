...
public struct WriteFileToolExecutor: ToolExecutor {
    public let definition = ToolDefinition(
        ...
        description: "Write a complete UTF-8 text file. Workspace-relative paths inside the session directory are allowed by default; other explicit targets require elevated approval.",
        inputSchema: ToolInputSchema(jsonSchema: """
        {"type":"object","properties":{"path":{"type":"string"},"content":{"type":"string"},"createIntermediateDirectories":{"type":"boolean"},"overwrite":{"type":"boolean"}},"required":["path","content"]}
        """)
    )
    ...
    public func execute(_ request: ToolExecutionRequest) async -> ToolExecutionResult {
        do {
            let arguments = try JSONDecoder().decode(WriteFileArguments.self, from: Data(request.argumentsJSON.utf8))
