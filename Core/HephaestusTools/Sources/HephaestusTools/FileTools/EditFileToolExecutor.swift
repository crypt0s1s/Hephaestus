...
public struct EditFileToolExecutor: ToolExecutor {
    public let definition = ToolDefinition(
        ...
        description: "Replace exact UTF-8 text in a file. The edit fails unless the expected occurrence count matches.",
        inputSchema: ToolInputSchema(jsonSchema: """
        {"type":"object","properties":{"path":{"type":"string"},"oldText":{"type":"string"},"newText":{"type":"string"},"expectedOccurrences":{"type":"integer","minimum":1}},"required":["path","oldText","newText"]}
        """)
    )
    ...
    public func execute(_ request: ToolExecutionRequest) async -> ToolExecutionResult {
        do {
            let arguments = try JSONDecoder().decode(EditFileArguments.self, from: Data(request.argumentsJSON.utf8))
            ...
            let metadata = [
                ...
                "byteDelta": String(Data(arguments.newText.utf8).count - Data(arguments.oldText.utf8).count),
                "location": resolved.location.rawValue
            ]
            return ToolExecutionResult(
                status: .succeeded,
                outputText: "Edited \(metadata["path"] ?? target.path). Replaced \(arguments.expectedOccurrences) occurrence(s).",
                metadata: metadata
            )
        } catch {
    ...
    private func edit(arguments: EditFileArguments, target: URL) throws {
        ...
        guard count == arguments.expectedOccurrences else {
            throw FileToolFailure.unexpectedOccurrenceCount(expected: arguments.expectedOccurrences, actual: count)
        }
