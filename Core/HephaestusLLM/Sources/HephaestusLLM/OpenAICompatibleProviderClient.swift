...
public struct OpenAICompatibleProviderClient: ProviderClient {
        ...
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    ...
                    let eventStream = transport.stream(request: transportRequest)
                    ...
                    for try await event in eventStream {
                        ...
                        case .data(let data):
                            ...
                            for chunk in try parser.parse(data) {
                                continuation.yield(chunk)
                            }
                        }
                    }

                    if let failedStatusCode {
                    ...
                    let chunks = try request.stream
                        ? parser.finish()
                        : Self.parseChatCompletionsResponse(responseBody, providerRequestID: request.id)
                    for chunk in chunks {
                        continuation.yield(chunk)
                    }
                    continuation.finish()
    ...
    public func makeTransportRequest(for request: ProviderRequest) throws -> OpenAICompatibleTransportRequest {
    ...
    private static func parseChatCompletionsResponse(
        _ data: Data,
...
public final class URLSessionOpenAICompatibleTransport: OpenAICompatibleTransport, @unchecked Sendable {
...
private extension Optional where Wrapped == String {
    var providerFinishReason: ProviderFinishReason? {
        switch self {
        case .some("stop"):
            return .stop
        case .some("length"):
