...
public enum RuntimeComposition {
    ...
    private static func makePersistent(
        ...
        let eventHub = RuntimeEventHub()
        let makeRun: @Sendable (PersistedSession) -> Run = { session in
            let profile = AgentProfile(
                name: agentName,
            ...
            return Run(
                id: session.id,
                agent: Agent(name: agentName, profile: profile),
        ...
        let createSession = DefaultCreateSessionUseCase(store: store, runStore: runStore, makeRun: makeRun)
        ...
        return PersistentRuntimeHarness(
            ...
            validateProviderSettings: OpenAICompatibleProviderSettingsValidator(transport: validationTransport),
...
private struct ConfigurationFailingProviderClient: ProviderClient {
    let reason: String
...
public struct OpenAICompatibleProviderSettingsValidator: ValidateProviderSettingsUseCase {
    ...
    public func validateProviderSettings(_ draft: ProviderSettingsDraft) async -> ProviderSettingsValidationResult {
