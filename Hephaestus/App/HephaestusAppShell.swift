...
actor PersistentAppRuntime:
  ...
  init(
    ...
    self.store = store
    providerOverride = try RuntimeProviderConfiguration.resolveExplicitOverride(
  ...
  func streamUserMessage(runID: UUID, text: String) async throws -> AsyncThrowingStream<
    RuntimeEvent, Error
  > {
  ...
  func saveProviderSettings(_ draft: ProviderSettingsDraft, validatedAt: Date?) async throws {
    ...
    try await (await currentHarness()).providerSettings.saveProviderSettings(
      resolvedDraft, validatedAt: validatedAt)
    harness = nil
  ...
  func validateProviderSettings(_ draft: ProviderSettingsDraft) async
    ...
    do {
      let resolvedDraft = try await draftWithPreservedKey(draft)
      return await (await currentHarness()).validateProviderSettings.validateProviderSettings(
        resolvedDraft)
    } catch {
  ...
  private func makeHarness() async -> PersistentRuntimeHarness {
    ...
    case .openAICompatible(let configuration):
      return RuntimeComposition.makePersistentOpenAICompatible(
  ...
  private func draftWithPreservedKey(_ draft: ProviderSettingsDraft) async throws
