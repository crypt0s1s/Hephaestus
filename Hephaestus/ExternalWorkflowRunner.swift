    ...
    private func runProcess(
        ...
        await withCheckedContinuation { continuation in
            ...
            process.arguments = [
                ...
                "--scratch-path", SwiftPackageScratchPath.url(for: URL(fileURLWithPath: packagePath, isDirectory: true)).path,
                entry,
                "run",
                "--input", inputURL.path
            ]
...
private extension WorkflowStepRecordStatus {
    nonisolated init(eventStatus: ExternalWorkflowEventStatus?) {
        switch eventStatus {
        case .pending:
            self = .pending
        case .inProgress:
