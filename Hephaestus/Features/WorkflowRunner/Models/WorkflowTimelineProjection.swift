...
nonisolated struct WorkflowTimelineProjection: Equatable {
    ...
    nonisolated static func make(records: [WorkflowStepRecord]) -> WorkflowTimelineProjection {
        ...
        let cycleRecords = hierarchicalRecords
    ...
    private nonisolated func visibleNodes(from node: WorkflowTimelineNode, collapsedIDs: Set<String>) -> [WorkflowTimelineNode] {
    ...
    nonisolated static func collapsedIDs(defaultExpandedIDs: Set<String>, manuallyExpandedIDs: Set<String>, manuallyCollapsedIDs: Set<String>, in nodes: [WorkflowTimelineNode]) -> Set<String> {
    ...
    private nonisolated static func setupRecords(in records: [WorkflowStepRecord]) -> [WorkflowStepRecord] {
        records.filter { record in
            record.hierarchy?.groupID == "setup" || (record.hierarchy?.cycleIndex == nil && record.hierarchy?.parentID == nil)
        }
    }

    private nonisolated static func makeSetupNode(records: [WorkflowStepRecord]) -> WorkflowTimelineNode {
    ...
    private nonisolated static func makeCycleNode(cycleIndex: Int, records: [WorkflowStepRecord], hasLaterCycle: Bool) -> WorkflowTimelineNode {
        let directRecords = records.filter { ($0.hierarchy?.parentID).isNilOrEmpty && ($0.hierarchy?.depth ?? 1) <= 1 }
        let parentIDs = records
            .compactMap { $0.hierarchy?.parentID }
            .filter { !$0.isEmpty }
        let uniqueParentIDs = Array(Set(parentIDs)).sorted { lhs, rhs in
            sortKey(parentSortKey(parentID: lhs, records: records), isLessThan: parentSortKey(parentID: rhs, records: records))
        }

        var children = leafNodes(from: directRecords, fallbackDepth: 1)
        for parentID in uniqueParentIDs {
            children.append(makeParentNode(parentID: parentID, records: records.filter { $0.hierarchy?.parentID == parentID }))
        }
        children.sort(by: nodeSort)

        let outcome = aggregateCycleOutcome(records: records, children: children, hasLaterCycle: hasLaterCycle)
        return WorkflowTimelineNode(
            id: "cycle-\(cycleIndex)",
            label: cycleIndex == 0 ? "Cycle 0 - Initial implementation" : "Cycle \(cycleIndex) - Fix cycle \(cycleIndex)",
            detail: aggregateDetail(for: children),
    ...
    private nonisolated static func makeParentNode(parentID: String, records: [WorkflowStepRecord]) -> WorkflowTimelineNode {
        ...
        if !clippedRecords.isEmpty {
            children.append(WorkflowTimelineNode(
                id: "\(parentID)-depth-summary",
                label: "Nested work",
                detail: "\(clippedRecords.count) deeper update\(clippedRecords.count == 1 ? "" : "s") summarized.",
                status: aggregateStatus(clippedRecords.map { TimelineDisplayStatus(recordStatus: $0.status) }),
    ...
    private nonisolated static func leafNodes(from records: [WorkflowStepRecord], fallbackDepth: Int) -> [WorkflowTimelineNode] {
    ...
    private nonisolated static func aggregateCycleOutcome(records: [WorkflowStepRecord], children: [WorkflowTimelineNode], hasLaterCycle: Bool) -> WorkflowStepRecordCycleOutcome? {
    ...
    private nonisolated static func status(for outcome: WorkflowStepRecordCycleOutcome?, childStatuses: [TimelineDisplayStatus]) -> TimelineDisplayStatus {
        switch outcome {
        case .running:
            return .inProgress
        case .succeeded:
    ...
    private nonisolated static func aggregateStatus(_ statuses: [TimelineDisplayStatus]) -> TimelineDisplayStatus {
    ...
    private nonisolated static func aggregateDetail(for children: [WorkflowTimelineNode]) -> String? {
        ...
        if children.allSatisfy({ $0.kind == .leaf && $0.label.localizedCaseInsensitiveContains("Reviewer") }),
    ...
    private nonisolated static func defaultExpandedIDs(for nodes: [WorkflowTimelineNode]) -> Set<String> {
    ...
    private nonisolated static func recordSort(_ lhs: WorkflowStepRecord, _ rhs: WorkflowStepRecord) -> Bool {
    ...
    private nonisolated static func nodeSort(_ lhs: WorkflowTimelineNode, _ rhs: WorkflowTimelineNode) -> Bool {
    ...
    private nonisolated static func parentSortKey(parentID: String, records: [WorkflowStepRecord]) -> [Int] {
        records
