...
struct WorkflowTimelineProjectionTests {
    ...
    func projectionGroupsSetupCyclesAndReviewersInCycleMajorOrder() throws {
        let records = [
            record("setup-plan", "Plan validation", .succeeded, groupID: "setup", cycle: nil, depth: 1, phase: 0, sequence: 0),
            record("cycle-1-implementer", "Step 1 - Implementer", .inProgress, groupID: "cycle-1", cycle: 1, depth: 1, phase: 10, sequence: 6, outcome: .running),
            record("cycle-0-feedback", "Step 4 - Feedback relay", .succeeded, groupID: "cycle-0", cycle: 0, depth: 1, phase: 40, sequence: 5, outcome: .needsFix),
            record("cycle-0-reviewer-b", "Step 3.2 - Reviewer B", .failed, groupID: "cycle-0", parentID: "cycle-0-review", cycle: 0, depth: 2, phase: 31, sequence: 4),
            record("cycle-0-build", "Step 2 - Build", .succeeded, groupID: "cycle-0", cycle: 0, depth: 1, phase: 20, sequence: 2),
            record("cycle-0-reviewer-a", "Step 3.1 - Reviewer A", .failed, groupID: "cycle-0", parentID: "cycle-0-review", cycle: 0, depth: 2, phase: 30, sequence: 3),
            record("cycle-0-implementer", "Step 1 - Implementer", .succeeded, groupID: "cycle-0", cycle: 0, depth: 1, phase: 10, sequence: 1)
        ]
        ...
        #expect(cycle0.children.map { $0.label } == [
            "Step 1 - Implementer",
            "Step 2 - Build",
            "Step 3 - Review",
    ...
    func projectionDistinguishesNeedsFixFromTerminalFailureDefaults() throws {
        let continuingRecords = [
            record("cycle-0-reviewer", "Reviewer A", .failed, groupID: "cycle-0", parentID: "cycle-0-review", cycle: 0, depth: 2, phase: 30, sequence: 0),
            record("cycle-0-feedback", "Step 4 - Feedback relay", .succeeded, groupID: "cycle-0", cycle: 0, depth: 1, phase: 40, sequence: 1, outcome: .needsFix),
            record("cycle-1-implementer", "Step 1 - Implementer", .inProgress, groupID: "cycle-1", cycle: 1, depth: 1, phase: 10, sequence: 2, outcome: .running)
        ]
        ...
        let terminal = WorkflowTimelineProjection.make(records: [
            record("cycle-0-build", "Step 2 - Build", .failed, groupID: "cycle-0", cycle: 0, depth: 1, phase: 20, sequence: 0, outcome: .terminalFailed)
        ])
    ...
    func projectionCollapseOverridesAndDepthTwoSummaryAreStable() throws {
        let projection = WorkflowTimelineProjection.make(records: [
            record("cycle-0-reviewer", "Reviewer A", .succeeded, groupID: "cycle-0", parentID: "cycle-0-review", cycle: 0, depth: 2, phase: 30, sequence: 0, outcome: .succeeded),
            record("cycle-0-deep-attempt", "Attempt 1", .succeeded, groupID: "cycle-0", parentID: "cycle-0-review", cycle: 0, depth: 3, phase: 31, sequence: 1)
        ])
        ...
        #expect(review.children.contains { $0.kind == WorkflowTimelineNode.Kind.summary && $0.depth == 2 })
    }

    @Test
    func legacyRecordsKeepFlatFallbackRows() {
        let rows = TimelineDisplayRowsBuilder(timeline: "", stepRecords: [
            WorkflowStepRecord(id: "b", title: "Second", status: .pending, summary: "two", sortOrder: 2),
            WorkflowStepRecord(id: "a", title: "First", status: .succeeded, summary: "one", sortOrder: 1)
        ]).rows
