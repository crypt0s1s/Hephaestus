...
struct PersistentRuntimeTests {
    ...
    func providerValidationUsesInjectedTransportForSuccessAndFailure() async throws {
        ...
        let success = await successValidator.validateProviderSettings(ProviderSettingsDraft(
            baseURLString: "https://provider.test/v1",
            apiKey: "token",
            model: "model"
        ))
        ...
        let failure = await failureValidator.validateProviderSettings(ProviderSettingsDraft(
            baseURLString: "https://provider.test/v1",
            apiKey: "token",
            model: "model"
        ))
    ...
    func contextTraceRecordsIncludedAndExcludedMessagesWithSmallLimit() async throws {
        ...
        _ = try await collect(harness.streamUserMessage.streamUserMessage(runID: session.id, text: "one"))
        _ = try await collect(harness.streamUserMessage.streamUserMessage(runID: session.id, text: "two"))
        _ = try await collect(harness.streamUserMessage.streamUserMessage(runID: session.id, text: "three"))
    ...
    func runInspectionContainsOrderedEventsProviderSummaryContextAndFailure() async throws {
        ...
        #expect(inspection.orderedEvents.map(\.sequence) == inspection.orderedEvents.map(\.sequence).sorted())
        #expect(inspection.orderedEvents.contains(where: { $0.kind == .turnFailed && $0.error?.contains("HTTP 500") == true }))
        #expect(inspection.session.providerRequests.contains(where: {
